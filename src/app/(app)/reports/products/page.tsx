...
import { reportByProduct, resolvePeriod, type ProductOrder } from '@/server/queries/reports';
...
export const metadata = { title: 'تقرير الأصناف' };
...
export default async function ProductsReportPage({ searchParams }: { searchParams: Promise<SP> }) {
  const profile = await requireProfile();
  const showProfit = canSeeProfit(profile.role);
  const sp = await searchParams;
  const { from, to } = resolvePeriod(sp);
  const order: ProductOrder = sp.order === 'qty' ? 'qty' : sp.order === 'profit' && showProfit ? 'profit' : 'revenue';
  const page = Math.max(1, Number(sp.page ?? 1) || 1);

  const { rows, total_count } = await reportByProduct(from, to, order, PAGE_SIZE, (page - 1) * PAGE_SIZE);

  const orders: { value: ProductOrder; label: string }[] = [
    { value: 'revenue', label: 'الإيراد' },
    { value: 'qty', label: 'الكمية' },
    ...(showProfit ? [{ value: 'profit' as const, label: 'الربح' }] : []),
  ];

  const orderHref = (o: ProductOrder) => {
  ...
  return (
    <div id="report-print">
      ...
      <PageHeader
        title="تقرير حسب الصنف"
        description={`${fmtDateShort(`${from}T12:00:00`)} — ${fmtDateShort(`${to}T12:00:00`)} — ${total_count} صنف`}
      />

      <PeriodFilter>
        <ExportButton reportKey="products" from={from} to={to} />
      </PeriodFilter>
      ...
      <Card>
        {rows.length === 0 ? (
          <EmptyState title="لا مبيعات أصناف في هذه الفترة" description="جرّب فترة أخرى من شريط الفلترة" />
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-ink-200 text-xs text-ink-500">
                  ...
                  <th className="px-4 py-3 text-start font-bold">الكمية المباعة</th>
                  <th className="px-4 py-3 text-end font-bold">عدد الفواتير</th>
                  <th className="px-4 py-3 text-end font-bold">الإيراد الصافي</th>
                  <th className="px-4 py-3 text-end font-bold">المرتجع</th>
              ...
              <tbody className="divide-y divide-ink-100">
                {rows.map((r) => (
                  <tr key={r.product_id} className="transition-colors hover:bg-primary-50/40">
                    <td className="px-4 py-3">
                      <Link href={`/products/${r.product_id}`} className="font-bold text-primary-700 hover:underline">
                        {r.product_name}
                      </Link>
                    </td>
                    <td className="px-4 py-3">
                      <p className="tnum font-bold">{r.qty_units} حبة</p>
                      {r.units_per_carton && r.units_per_carton > 1 ? (
                        <p className="text-xs text-ink-500">{formatQty(r.qty_units, r.units_per_carton)}</p>
                      ) : null}
                    </td>
                    <td className="tnum px-4 py-3 text-end">{r.invoices}</td>
                    <td className="px-4 py-3 text-end"><Money value={r.net_revenue} /></td>
